...
train_list1, train_label1, val_list1, val_label1, test_list, test_label = getLSPDataset(0.9, 0.05)
train_list, train_label = data_augmentation(train_list1, train_label1, times = 2)
...
train_data = np.loadtxt('train_pred_vF_1.txt')
print(train_data.shape)
train_data = train_data.reshape((-1,2,14))
print(train_data.shape)
...
val_data = np.loadtxt('val_pred_vF_1.txt')
print(val_data.shape)
val_data = val_data.reshape((-1,2,14))
print(val_data.shape)
...
for i in range(int(val_label1.shape[0])):
    torsor_xy = val_label1[i, :, 9] - val_label1[i, :, 2]
...
def get_acc(targets, predictions, torsor_frac):
    ...
    wrist_acc = np.sum(valid_mask[:, 6] + valid_mask[:, 11]) / (int(joint_dist.shape[0]) * 2)
    elbow_acc = np.sum(valid_mask[:, 7] + valid_mask[:, 10]) / (int(joint_dist.shape[0]) * 2)
    ankle_acc = np.sum(valid_mask[:, 0] + valid_mask[:, 5]) / (int(joint_dist.shape[0]) * 2)
    knee_acc = np.sum(valid_mask[:, 1] + valid_mask[:, 4]) / (int(joint_dist.shape[0]) * 2)
    return accuracy, wrist_acc, elbow_acc, ankle_acc, knee_acc

torsor_xy = val_label1[:, :, 9] - val_label1[:, :, 2] #distance left_shoulder <-> right_hip in xy, result (N, 2)
...
for thresh in thresh_list:
    cur_acc, cur_w_acc, cur_e_acc, cur_a_acc, cur_k_acc = get_acc(val_label1, val_data, torsor_dist * thresh)
    acc.append(cur_acc)
    w_acc.append(cur_w_acc)
    e_acc.append(cur_e_acc)
    a_acc.append(cur_a_acc)
    k_acc.append(cur_k_acc)
print(acc, w_acc, e_acc, a_acc, k_acc)
...
fig.savefig('AlexNet_acc_LSP_thresh.png')
plt.show()
...
plt.xlabel('torso fraction')
plt.title("Learning rate: " + str(0.00005))
fig.savefig('AlexNet_acc_arm_LSP_thresh.png')
plt.show()

fig = plt.figure()
plt.plot(thresh_list, np.squeeze(a_acc), label = "ankle")
plt.plot(thresh_list, np.squeeze(k_acc), label = "knee")
plt.legend()
plt.ylabel('accuracy')
plt.xlabel('torso fraction')
plt.title("Learning rate: " + str(0.00005))
fig.savefig('AlexNet_acc_leg_LSP_thresh.png')
plt.show()
