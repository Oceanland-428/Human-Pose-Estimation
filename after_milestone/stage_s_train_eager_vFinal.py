...
train_list, train_label = data_augmentation(train_list, train_label, times = 2)
...
def get_loss_acc(targets, predictions, torsor_frac):
    ...
    wrist_acc = tf.reduce_sum(valid_mask[:, 6] + valid_mask[:, 11]) / (int(joint_dist.shape[0]) * 2)
    elbow_acc = tf.reduce_sum(valid_mask[:, 7] + valid_mask[:, 10]) / (int(joint_dist.shape[0]) * 2)
    ankle_acc = tf.reduce_sum(valid_mask[:, 0] + valid_mask[:, 5]) / (int(joint_dist.shape[0]) * 2)
    knee_acc = tf.reduce_sum(valid_mask[:, 1] + valid_mask[:, 4]) / (int(joint_dist.shape[0]) * 2)
    return loss.numpy(), accuracy.numpy(), wrist_acc.numpy(), elbow_acc.numpy(), ankle_acc.numpy(), knee_acc.numpy()
...
def loss_accuracy_1(model, inputs, targets, thresh, is_training):
    ...
    predictions = model.predict(inputs, is_training) # (N, 2, 14) in original scale
    loss, accuracy, wrist_acc, elbow_acc, ankle_acc, knee_acc = get_loss_acc(targets, predictions, torsor_frac)
    
    return loss, accuracy, wrist_acc, elbow_acc, ankle_acc, knee_acc
...
for epoch in range(100):
    with tf.device('/device:GPU:0'):
        ...
        for batch in range(n_batch):
            ...
            _, cur_tr_acc_1, _, _, _, _ = loss_accuracy_1(model1, batch_X, batch_Y, thresh, False)
        ...
        val_loss_1, val_acc_1, wrist_acc_1, elbow_acc_1, ankle_acc_1, knee_acc_1 = loss_accuracy_1(model1, val_X, val_Y, thresh, False)
        val_acc_list_1.append(val_acc_1)
        wrist_acc_list_1.append(wrist_acc_1)
        elbow_acc_list_1.append(elbow_acc_1)
        ankle_acc_list_1.append(ankle_acc_1)
        knee_acc_list_1.append(knee_acc_1)
        val_loss_1 /= int(val_Y.shape[0])
        val_cost_1.append(val_loss_1)
        print("val loss 1: ", val_loss_1, "val accuracy 1: ", val_acc_1)
        print("wrist acc 1: ", wrist_acc_1, "elbow acc 1: ", elbow_acc_1, "ankle acc 1: ", ankle_acc_1, "knee acc 1: ", knee_acc_1)
...
for epoch in range(30):
    with tf.device('/device:GPU:0'):
        ...
        for batch in range(n_batch):
            ...
            predictions_1 = model1.predict(batch_X_1, is_training = False)
            inputs_2, b_array_1 = images_crop(batch_X_1, predictions_1, batch_Y_1)
            _, cur_tr_acc_2, _, _, _, _ = loss_accuracy_s(model2, inputs_2, batch_Y_1, thresh, predictions_1, b_array_1, False)
        ...
        for batch in range(val_n_batch):
            ...
            cur_val_loss_2, cur_val_acc_2, cur_wrist_acc_2, cur_elbow_acc_2, cur_ankle_acc_2, cur_knee_acc_2 = loss_accuracy_s(model2, val_batch_X, val_batch_Y, thresh, val_batch_predictions_1, val_batch_b_array_1, False)
            ...
            ankle_acc_2 += cur_ankle_acc_2 * int(val_batch_X.shape[0])
            knee_acc_2 += cur_knee_acc_2 * int(val_batch_X.shape[0])
            val_loss_2 += cur_val_loss_2
        val_acc_2 /= int(val_Y.shape[0])
        wrist_acc_2 /= int(val_Y.shape[0])
        elbow_acc_2 /= int(val_Y.shape[0])
        ankle_acc_2 /= int(val_Y.shape[0])
        knee_acc_2 /= int(val_Y.shape[0])
        val_acc_list_2.append(val_acc_2)
        wrist_acc_list_2.append(wrist_acc_2)
        elbow_acc_list_2.append(elbow_acc_2)
        ankle_acc_list_2.append(ankle_acc_2)
        knee_acc_list_2.append(knee_acc_2)
        val_loss_2 /= int(val_Y.shape[0])
        val_cost_2.append(val_loss_2)
        print("val loss 2: ", val_loss_2, "val accuracy 2: ", val_acc_2)
        print("wrist acc 2: ", wrist_acc_2, "elbow acc 2: ", elbow_acc_2, "ankle acc 2: ", ankle_acc_2, "knee acc 2: ", knee_acc_2)
...
with open('ankle_acc_list_1.txt', 'w') as filehandle:  
    for listitem in np.array(ankle_acc_list_1):
        filehandle.write('%s\n' % listitem)
        
with open('knee_acc_list_1.txt', 'w') as filehandle:  
    for listitem in np.array(knee_acc_list_1):
        filehandle.write('%s\n' % listitem)
...
with open('ankle_acc_list_2.txt', 'w') as filehandle:  
    for listitem in np.array(ankle_acc_list_2):
        filehandle.write('%s\n' % listitem)
        
with open('knee_acc_list_2.txt', 'w') as filehandle:  
    for listitem in np.array(knee_acc_list_2):
        filehandle.write('%s\n' % listitem)
